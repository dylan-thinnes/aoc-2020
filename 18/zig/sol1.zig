...
pub fn process_stream(lexemes: *LexemeStream, allocator: *std.mem.Allocator) *Stack {
    ...
    stack.* = Stack{ .val = 0, .op = Op.add, .prev = null };
    ...
    while (lexeme != null) {
        switch (lexeme.?) {
            ...
            .op    => |op| stack.*.op = op,
            .open  => |_| push_stack(&stack, allocator),
            .close => |_| pop_stack(&stack),
        }
...
pub fn push_stack(stackp: **Stack, allocator: *std.mem.Allocator) void {
    var old_stack: *Stack = stackp.*;
    stackp.* = allocator.*.create(Stack) catch unreachable;
    stackp.*.* = Stack{ .val = 0, .op = Op.add, .prev = old_stack };
}
